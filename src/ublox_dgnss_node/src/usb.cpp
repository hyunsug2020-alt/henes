...
namespace
{
std::string usb_device_token_to_board_prefix(const std::string & usb_device_token)
{
  auto dash = usb_device_token.find('-');
  if (dash == std::string::npos || dash + 1 >= usb_device_token.size()) {
    return "";
  }

  std::string port_chain = usb_device_token.substr(dash + 1);
  auto last_dot = port_chain.rfind('.');
  if (last_dot == std::string::npos) {
    return port_chain;
  }
  return port_chain.substr(0, last_dot);
}

std::string resolve_device_actual_board_prefix(const std::string & device_actual)
{
  if (device_actual.empty()) {
    return "";
  }
  ...
  std::error_code ec;
  fs::path resolved_dev = fs::canonical(fs::path(device_actual), ec);
  if (ec) {
    throw std::string("Error resolving DEVICE_ACTUAL path: ") + device_actual;
  }

  fs::path sysfs_path = fs::canonical(
    fs::path("/sys/class/tty") / resolved_dev.filename() / "device",
    ec);
  if (ec) {
    throw std::string("Error resolving DEVICE_ACTUAL sysfs path: ") + resolved_dev.string();
  }

  for (fs::path current = sysfs_path; !current.empty(); current = current.parent_path()) {
    ...
    std::string usb_device_token = (colon == std::string::npos) ? name : name.substr(0, colon);
    if (usb_device_token.find('-') == std::string::npos) {
      continue;
    }

    std::string board_prefix = usb_device_token_to_board_prefix(usb_device_token);
    if (!board_prefix.empty()) {
      return board_prefix;
    }
  }

  throw std::string("Error extracting USB board prefix from DEVICE_ACTUAL: ") + resolved_dev.string();
}

std::string get_libusb_board_prefix(libusb_device * device)
{
  std::array<uint8_t, 8> port_numbers{};
  int port_count = libusb_get_port_numbers(device, port_numbers.data(), port_numbers.size());
  if (port_count < 1) {
    return "";
  }

  std::ostringstream board_prefix;
  int end_index = (port_count > 1) ? (port_count - 1) : port_count;
  for (int index = 0; index < end_index; ++index) {
    if (index > 0) {
      board_prefix << '.';
    }
    board_prefix << static_cast<int>(port_numbers[static_cast<size_t>(index)]);
  }
  return board_prefix.str();
}
}  // namespace

Connection::Connection(
  int vendor_id, const std::vector<uint16_t> & product_ids, std::string serial_str,
  std::string device_actual, ublox_dgnss::DeviceFamily device_family, int log_level)
{
...
libusb_device_handle * Connection::open_device_with_serial_string(
  ...
  std::string device_actual,
  char * serial_num_string)
{
  libusb_device_handle * devHandle = nullptr;
  int rc = 0;
  if (serial_num_string != nullptr) {
    serial_num_string[0] = '\0';
  }

  std::string requested_board_prefix;
  if (!device_actual.empty()) {
    requested_board_prefix = resolve_device_actual_board_prefix(device_actual);
  }
  ...
  for (ssize_t i = 0; i < deviceCount; i++) {
    ...
    if (!requested_board_prefix.empty()) {
      std::string candidate_board_prefix = get_libusb_board_prefix(device);
      if (candidate_board_prefix != requested_board_prefix) {
        continue;
      }
    }
...
bool Connection::open_device()
{
  ...
  devh_ = open_device_with_serial_string(
    ctx_, vendor_id_, product_ids_, serial_str_, device_actual_,
    serial_num_string);
  if (!devh_) {
