...
public:
      ...
      .automatically_declare_parameters_from_overrides(true))
  {
    ...
    // Check device family first, then startup device selection parameters.
    ...
    check_for_device_actual_param(parameters_client);
    ...
    for (auto name : list_param_result.names) {
      ...
      if (strcmp(name.c_str(), DEVICE_ACTUAL_PARAM_NAME.c_str()) == 0) {
        continue;
      }
    ...
    RCLCPP_DEBUG(
      get_logger(), "Make USB Connection - Device: %s, Product IDs: %zu, Serial: '%s', Actual: '%s'",
      device_info.description.c_str(), device_info.product_ids.size(),
      serial_str_.c_str(), device_actual_.c_str());
    usbc_ = std::make_shared<usb::Connection>(
      U_BLOX_AG_VENDOR_ID, device_info.product_ids,
      serial_str_, device_actual_, device_family_);
...
// once the usb is initialised this timer is disabled
  ...
  std::string device_actual_;
  const std::string DEVICE_ACTUAL_PARAM_NAME = "DEVICE_ACTUAL";
  ...
  UBLOX_DGNSS_NODE_LOCAL
  void check_for_device_actual_param(rclcpp::SyncParametersClient::SharedPtr param_client)
  {
    device_actual_.clear();

    if (!param_client->has_parameter(DEVICE_ACTUAL_PARAM_NAME)) {
      RCLCPP_INFO(
        this->get_logger(), "Parameter %s not found, no USB board lock will be used",
        DEVICE_ACTUAL_PARAM_NAME.c_str());
      return;
    }

    device_actual_ = param_client->get_parameter<std::string>(DEVICE_ACTUAL_PARAM_NAME);
    if (device_actual_.empty()) {
      RCLCPP_INFO(
        this->get_logger(), "Parameter %s is empty, no USB board lock will be used",
        DEVICE_ACTUAL_PARAM_NAME.c_str());
      return;
    }
    ...
    RCLCPP_INFO(
      this->get_logger(), "Parameter %s found with value: %s",
      DEVICE_ACTUAL_PARAM_NAME.c_str(), device_actual_.c_str());
  }
