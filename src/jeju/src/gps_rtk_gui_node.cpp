...
public:
  ...
  : Node("gps_rtk_gui_node")
  {
    ...
    this->declare_parameter("imu_topic", "/handsfree/imu");
    this->declare_parameter("window_name", "HENES Dual GPS / IMU Monitor");
    ...
    heading_acc_sub_ = this->create_subscription<std_msgs::msg::Float64>(
      heading_accuracy_topic_, sensor_qos,
      [this](const std_msgs::msg::Float64::SharedPtr msg) {
...
private:
  ...
  {
    const bool fresh = has_imu_ && (this->now() - last_imu_time_).seconds() <= topic_timeout_sec_;
    cv::rectangle(img, rect, kPanel, cv::FILLED);
    cv::rectangle(img, rect, kBorder, 2);
    ...
    char buf[128];
    const int col1 = rect.x + 26, col2 = rect.x + 260, col3 = rect.x + 510;
    const int y0 = rect.y + 80, dy = 46;

    // 자세
    cv::putText(img, "ATTITUDE", {col1, y0}, cv::FONT_HERSHEY_SIMPLEX, 0.60, kMuted, 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf), "Roll  %+7.2f deg", imu_roll_);
    cv::putText(img, buf, {col1, y0 + dy}, cv::FONT_HERSHEY_SIMPLEX, 0.72, kText, 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf), "Pitch %+7.2f deg", imu_pitch_);
    cv::putText(img, buf, {col1, y0 + dy*2}, cv::FONT_HERSHEY_SIMPLEX, 0.72, kText, 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf), "Yaw   %+7.2f deg", imu_yaw_);
    cv::putText(img, buf, {col1, y0 + dy*3}, cv::FONT_HERSHEY_SIMPLEX, 0.72, kText, 1, cv::LINE_AA);

    // 각속도
    cv::putText(img, "GYRO (deg/s)", {col2, y0}, cv::FONT_HERSHEY_SIMPLEX, 0.60, kMuted, 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf), "X %+7.2f", imu_gx_);
    cv::putText(img, buf, {col2, y0 + dy}, cv::FONT_HERSHEY_SIMPLEX, 0.72, kText, 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf), "Y %+7.2f", imu_gy_);
    cv::putText(img, buf, {col2, y0 + dy*2}, cv::FONT_HERSHEY_SIMPLEX, 0.72, kText, 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf), "Z %+7.2f", imu_gz_);
    cv::putText(img, buf, {col2, y0 + dy*3}, cv::FONT_HERSHEY_SIMPLEX, 0.72, kText, 1, cv::LINE_AA);

    // 가속도
    cv::putText(img, "ACCEL (m/s²)", {col3, y0}, cv::FONT_HERSHEY_SIMPLEX, 0.60, kMuted, 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf), "X %+7.3f", imu_ax_);
    cv::putText(img, buf, {col3, y0 + dy}, cv::FONT_HERSHEY_SIMPLEX, 0.72, kText, 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf), "Y %+7.3f", imu_ay_);
    cv::putText(img, buf, {col3, y0 + dy*2}, cv::FONT_HERSHEY_SIMPLEX, 0.72, kText, 1, cv::LINE_AA);
    std::snprintf(buf, sizeof(buf), "Z %+7.3f", imu_az_);
    cv::putText(img, buf, {col3, y0 + dy*3}, cv::FONT_HERSHEY_SIMPLEX, 0.72, kText, 1, cv::LINE_AA);
  }
  ...
  void drawFrame()
  {
    ...
    cv::putText(
      img, "HENES Dual GPS / IMU Monitor", {36, 52}, cv::FONT_HERSHEY_DUPLEX, 1.2, kText, 2,
      cv::LINE_AA);
    cv::putText(
      img, "GPS1=Left/Rover  |  GPS2=Right/Base  |  Heading: /dual_f9p/*",
    ...
    // IMU 패널 (하단 전체 폭)
    drawImuPanel(img, {34, 822, 1432, 234});
