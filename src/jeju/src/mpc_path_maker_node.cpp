...
: Node("mpc_path_maker_node")
{
  ...
  // 기존 {N}_filtered.txt 파일 스캔 → 다음 번호 자동 부여
  int next_id = 1;
  for (const auto & entry : std::filesystem::directory_iterator(output_dir)) {
    const std::string fname = entry.path().filename().string();
    std::size_t pos = fname.find("_filtered.txt");
    if (pos == std::string::npos) { continue; }
    try {
      int n = std::stoi(fname.substr(0, pos));
      if (n >= next_id) { next_id = n + 1; }
    } catch (...) {}
  }
  std::string filename = std::to_string(next_id) + "_filtered.txt";
  ...
  gps_vel_sub_ = this->create_subscription<geometry_msgs::msg::TwistWithCovarianceStamped>(
    "/gnss_left/fix_velocity", 10, std::bind(&MpcPathMakerNode::gpsVelCb, this, std::placeholders::_1));
